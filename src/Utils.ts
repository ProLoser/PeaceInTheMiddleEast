...
export function classes(...args: (string | boolean | undefined | null | {[key: string]: boolean | undefined})[]) {
  let result = '';
  ...
  for (const arg of args) {
    ...
    if (type === 'string' || type === 'number') {
      ...
      result += ' ' + classes(...arg);
    } else if (type === 'object' && typeof arg === 'object') {
 ...
 * @param from
 * @param to
 * @return moveLabel is not returned if the move is blocked
 */
export function calculate(state: Game, from: number | Color | undefined | null, to: number, usedDice: UsedDie[] = []) {
  ...
  if (from === Color.White) { // white re-enter
    ...
    const dice = [...state.dice].sort((a, b) => a - b);
